<template>
  <div id="grmb-wrapper">
	 <div class="grmb-list" :style="{'-webkit-overflow-scrolling': scrollMode}">
...
export default {
	...
	methods:{
		...
		handleTopChange:function(status) {
			...
			if(_this.pagenow>1){
				...
				this.$http.get('http://192.168.0.56:8081/target/target_mbwwclbgr?userid=11204050220&function_id=20150101&ticket=B110E55DEECA941AEFDCF9CE72C2789D&pagenow='+_this.pagenow+'&pagesize='+_this.pagesize).then(res=>{
	...
	mounted(){
		var _this = this;
           this.$http.get('http://192.168.0.56:8081/target/target_mbwwclbgr?userid=11204050220&function_id=20150101&ticket=B110E55DEECA941AEFDCF9CE72C2789D&pagenow=1&pagesize=3').then(res=>{
            _this.data=res.data.content;
            // console.log(_this.data);
        })
	}
...
.grmb-list ul li span{
	display: inline-block;
	float: right;
	width: 1.8em;
	height: 1.8em;
}
.grmb-list ul li span img{
	width: 100%;
	height: 100%;
	border-radius: 50%;
