<!-- <template>
  <div id="ywc-wrapper">
	 <div class="ywc-list" :style="{'-webkit-overflow-scrolling': scrollMode}">
		<mt-loadmore :bottom-method="handleBottomChange" :top-method="handleTopChange" :bottom-all-loaded="allLoaded" :auto-fill="false" ref="loadmore">
			<ul v-for="(item,$index) in data">
				<li v-for="(list,index) in item.list" @click="completeXq(list.id)">
				<p>每天2小时学习c语言基础{{list.mbmc}}</p>
					<span>
						<img :src="list.tx">
					</span>
				</li>
...
export default {
	...
	methods:{
		completeXq(id){
            ...
            this.$http.get('http://192.168.0.101:8081/target/target_mbxq?userid=11204050220&function_id=20150101&ticket=7A459E71400C19A4A813994B79BC4E41&id='+id).then(res=>{
				...
				this.$router.push({path:'./../collegegoal/completedgoal-xq',query:{data:JSON.stringify(this.mbxq)}});
	      		// this.$router.push({path:'/collegegoaldetail/collegegoaldetail',query:{data:JSON.stringify(this.mbxq)}});
        	})
		},
		handleTopChange:function(status) {
			...
			if(_this.pagenow>1){
				...
				this.$http.get('http://192.168.0.101:8081/target/target_mbywclb?userid=11204050220&function_id=20150101&ticket=B110E55DEECA941AEFDCF9CE72C2789D&pagenow='+_this.pagenow+'&pagesize='+_this.pagesize).then(res=>{
	...
	mounted(){
		var _this = this;
           this.$http.get('http://192.168.0.101:8081/target/target_mbywclb?userid=11204050220&function_id=20150101&ticket=B110E55DEECA941AEFDCF9CE72C2789D&pagenow=1&pagesize=10').then(res=>{
            _this.data=res.data.content;
            console.log(_this.data);
        })
	}
